...
import { View, StyleSheet, Text, ScrollView, Image, TouchableOpacity, TextInput, Dimensions, Settings } from 'react-native';
...
const PerilCliente = (props: any) => {
            ...
            const response = await fetch(api.url,
                {
                    ...
                    body: JSON.stringify({ component:"usuario", type: 'getByKey' }),
                });
    ...
    const onChangeDate = async (tipo: string, text: string) => {
        if (!props.state.cliente) {
            props.state["cliente"] = {};
        }
        props.state["cliente"][tipo] = text;
        AsyncStorage.setItem("cliente", JSON.stringify(props.state["cliente"]));
        setState({ ...props });
    };
    ...
    return <View style={{ flex: 1 }}>
        ...
        {stateLoad ? loadPage() : <View>
            ...
            <TouchableOpacity onPress={() => openModal()}>
                <View style={{ position: "relative", width: "100%", height: 50, marginBottom: 5 }} >
                    ...
                    <View style={{ position: 'absolute', display: 'flex', flexDirection: 'row', alignItems: "center", top: '5%', left: '5%', width: '85%', height: '90%' }}>
                        <TextInput
                            style={{ ...styles.input }}
                            onChangeText={text => { props.onChangeCliente("FECHA_NACIMIENTO", text) }}
                            value={getDateFormated(props?.state?.cliente?.FECHA_NACIMIENTO)}
                            placeholderTextColor={tema.placeholder}
        ...
        <ModalCalendar onChangeDate={onChangeDate} fecha_nacimiento={true} modalVisible={modalVisible} closeModal={closeModal} fecha_inicial={props?.state?.cliente?.FECHA_NACIMIENTO} id_modal={"date"} navigation={navigation_} key={"FECHA_NACIMIENTO"} ></ModalCalendar>
    </View>
